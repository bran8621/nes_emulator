...
const INITIAL_STATUS: u8 = 0b100100;
...
impl CPU {
    ...
    fn load_data_and_update_flags(&mut self, mode: &AddressingMode, target: &mut u8) {
        ...
        *target = data;
        self.update_zero_and_negative_flags(*target);
    ...
    where F: Fn(u8, u8) -> u8,
    {
        let addr = self.get_operand_address(mode);
        let data = self.mem_read(addr);
        self.set_register_a(operation(data, self.register_a));
    }
        ...
        F: FnMut(&mut CPU),
    {
        ...
        loop {
            ...
            match code {
                ...
                /* RTI */
                0x40 => {
                    let flags_bits = self.stack_pop(); // Retrieve the flags from the stack
                    let flags = CpuFlags::from_bits_truncate(flags_bits); // Convert the retrieved bits into CpuFlags

                    // Update the status register with the retrieved flags
                    self.status = flags;
                    self.status.remove(CpuFlags::BREAK);
                    self.status.insert(CpuFlags::BREAK2);

                    self.program_counter = self.stack_pop_u16(); // Restore the program counter
                }
